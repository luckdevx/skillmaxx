...
**Maxx your AI agent skills. One command, zero config.**
...
1. Run `npx skillmaxx` in your project root
2. Your `package.json`, Gradle files, and config files are scanned to detect technologies
3. The best matching AI agent skills are selected from the audited skill registry
...
`skillmaxx` does not install directly from random upstream repositories at runtime.

Skills are synced by maintainers into the repository-local skill registry, scanned for prompt-injection and supply-chain risks, and recorded with SHA-256 hashes in a manifest. When you run `skillmaxx`, the CLI downloads only the skills your project needs from that curated registry, verifies every file against the manifest, and writes a `skills-lock.json` entry with the installed source and bundle hash.
