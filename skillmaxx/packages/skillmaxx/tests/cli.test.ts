...
describe("CLI", () => {
  ...
  it("clears the skillmaxx cache with --clear-cache", () => {
    ...
    try {
      ...
      ok(output.includes("Cleared skillmaxx cache"));
  ...
  describe("--dry-run", () => {
    ...
    it("accepts multiple -a flags for multiple agents", () => {
      writePackageJson(tmp.path, { dependencies: { react: "^19" } });
      const output = run(["--dry-run", "-a", "cursor", "-a", "claude-code"], tmp.path);
      ok(output.includes("Agents: cursor, claude-code"));
      ok(!output.includes("universal"));
    });
  });
});
