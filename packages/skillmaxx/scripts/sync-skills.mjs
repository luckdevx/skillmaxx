...
// approved markdown files inside `packages/skillmaxx/skills-registry/`.
//
...
const REVIEW_MODEL = process.env.SKILLMAXX_REVIEW_MODEL || "gpt-5.4";
...
async function downloadTarball(repo, sha, destFile) {
  ...
  const headers = { "User-Agent": "skillmaxx-sync" };
  if (GITHUB_TOKEN) headers.Authorization = `Bearer ${GITHUB_TOKEN}`;
...
function writeSkillToRegistry(manifest, skill) {
  ...
  if (skill.repoInfo) {
    entry.repoInfo = {
      stars: skill.repoInfo.stars,
      pushedAt: skill.repoInfo.pushedAt,
      sizeKB: skill.repoInfo.sizeKB,
    };
  }

  manifest.skills[skill.skillName] = entry;
}
...
async function main() {
  ...
  for (const [repo, skills] of byRepo) {
    ...
    let sha;
    let repoRoot;
    let directSkillDirs = null;
    let repoInfo = null;
    const tmpDir = mkdtempSync(join(tmpdir(), "skillmaxx-sync-"));
    ...
    try {
      ...
      repoInfo = null;
      try {
        repoInfo = await resolveRepoInfo(repo);
      } catch (err) {
        log(dim(`   ↳ repo info unavailable: ${err.message}`));
      }
      ...
      } else if (repoInfo && repoInfo.sizeKB > HEAVY_REPO_KB) {
        log(dim(`   ↳ repo is ${(repoInfo.sizeKB / 1024).toFixed(0)} MB — using per-file fetch`));
