...
function normalizeRepoUrl(rawUrl) {
  const fallback = "https://github.com/tokenmaxxing/skillmaxx";
...
/**
 * Collects commits affecting `packages/skillmaxx/` since the given tag.
 * @param {string|null} tag - Starting tag (all commits if `null`).
 * @returns {{ message: string, hash: string }[]}
 */
function getCommitsSinceTag(tag) {
  const range = tag ? `${tag}..HEAD` : "HEAD";
  const log = run(`git log ${range} --pretty=format:"%s|%h" -- packages/skillmaxx/`, {
    cwd: REPO_ROOT,
  });
  if (!log) return [];
  return log
