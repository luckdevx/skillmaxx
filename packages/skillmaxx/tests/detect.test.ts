...
`,
  ...
  it("detects Docker from Dockerfile", () => {
    writePackageJson(tmp.path);
    writeFile(tmp.path, "Dockerfile", "FROM node:22-alpine\nWORKDIR /app\n");
    const { detected } = detectTechnologies(tmp.path);
    ok(detected.some((t) => t.id === "docker"));
  });

  it("detects Docker from docker-compose.yml", () => {
    writePackageJson(tmp.path);
    writeFile(tmp.path, "docker-compose.yml", "version: '3'\nservices:\n  app:\n    build: .\n");
    const { detected } = detectTechnologies(tmp.path);
    ok(detected.some((t) => t.id === "docker"));
  });

  it("detects Docker from compose.yaml", () => {
    writePackageJson(tmp.path);
    writeFile(tmp.path, "compose.yaml", "services:\n  app:\n    image: node:22\n");
    const { detected } = detectTechnologies(tmp.path);
    ok(detected.some((t) => t.id === "docker"));
  });

  it("does not detect Docker without docker files", () => {
    writePackageJson(tmp.path);
    const { detected } = detectTechnologies(tmp.path);
    ok(!detected.some((t) => t.id === "docker"));
  });

  it("returns correct skills for Docker detection", () => {
    writePackageJson(tmp.path);
    writeFile(tmp.path, "Dockerfile", "FROM node:22-alpine");
    const { detected } = detectTechnologies(tmp.path);
    const docker = detected.find((t) => t.id === "docker");
    ok(docker);
    ok(docker.skills.includes("KentoShimizu/sw-agent-skills/dockerfile-best-practices"));
    ok(docker.skills.includes("KentoShimizu/sw-agent-skills/docker-compose-patterns"));
  });

  it("detects GitHub Actions from .github/workflows directory", () => {
    writePackageJson(tmp.path);
    writeFile(tmp.path, ".github/workflows/ci.yml", "name: CI\non: [push]\n");
    const { detected } = detectTechnologies(tmp.path);
    ok(detected.some((t) => t.id === "github-actions"));
  });

  it("does not detect GitHub Actions without .github/workflows", () => {
    writePackageJson(tmp.path);
    const { detected } = detectTechnologies(tmp.path);
    ok(!detected.some((t) => t.id === "github-actions"));
  });

  it("returns correct skills for GitHub Actions detection", () => {
    writePackageJson(tmp.path);
    writeFile(tmp.path, ".github/workflows/deploy.yml", "name: Deploy\non: [push]\n");
    const { detected } = detectTechnologies(tmp.path);
    const ga = detected.find((t) => t.id === "github-actions");
    ok(ga);
    ok(ga.skills.includes("KentoShimizu/sw-agent-skills/github-actions-workflow-design"));
    ok(ga.skills.includes("KentoShimizu/sw-agent-skills/github-fix-ci"));
  });
});
