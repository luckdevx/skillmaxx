...
function getRegistryRawBaseUrls(opts: InstallOptions): string[] {
  ...
  if (!version) {
    throw new Error("could not resolve skillmaxx package version for registry download");
  }

  return [
    `${DEFAULT_REGISTRY_RAW_BASE_URL_PREFIX}/v${version}/packages/skillmaxx/skills-registry`,
    `${DEFAULT_REGISTRY_RAW_BASE_URL_PREFIX}/main/packages/skillmaxx/skills-registry`,
  ];
}
...
export function getSkillmaxxCacheDir(): string {
  return (
    process.env.SKILLMAXX_CACHE_DIR || join(homedir(), ".cache", "skillmaxx", "skills-registry")
  );
}

export function clearSkillmaxxCache(): { cacheDir: string; removed: boolean } {
  const cacheDir = getSkillmaxxCacheDir();
...
function getCacheRegistryDir(entry: RegistryEntry): string {
  return join(getSkillmaxxCacheDir(), entry.bundleHash);
}
...
export function getRepoInfo(skillPath: string): RepoInfo | null {
  const { skillName } = parseSkillPath(skillPath);
  if (!skillName) return null;

  const registry = loadRegistry();
  const entry = registry?.skills[skillName];
  if (!entry?.repoInfo) return null;
...
function githubDownloadHeaders(url: string): HeadersInit {
  const headers: Record<string, string> = { "User-Agent": "skillmaxx" };
...
export async function installSkill(
  ...
  const command = `skillmaxx install ${skillPath}`;
  ...
  if (!registry) {
    return fail(
      `skills-registry index not found. Run 'pnpm sync:skills' in the skillmaxx package.`,
    );
  }
