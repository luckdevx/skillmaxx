...
`skillmaxx` scans your project, detects the technologies you use, and installs curated AI agent skills that make Cursor, Claude Code, and other AI coding assistants actually understand your stack.
...
```bash
npx skillmaxx --dry-run
```

### Claude Code summary

If `claude-code` is auto-detected or passed with `-a`, `skillmaxx` writes a `CLAUDE.md` file in your project root summarizing the markdown files installed under `.claude/skills`.
...
`skillmaxx` detects **50+ technologies** from your `package.json`, lockfiles, Gradle files, and config files:
...
Even without a framework, `skillmaxx` scans your file tree for web frontend signals (`.html`, `.css`, `.scss`, `.vue`, `.svelte`, `.jsx`, `.tsx`, `.twig`, `.blade.php`, etc.) and installs skills for frontend design, accessibility, and SEO.

## Combo Detection

When multiple technologies are used together, `skillmaxx` detects **technology combos** and adds specialized skills for the combination:
...
`skillmaxx` uses a curated skill registry under the hood — markdown files that teach AI assistants how to work with specific technologies, following best practices and patterns from the official maintainers.
...
CC-BY-NC-4.0 — Forked from [midudev/autoskills](https://github.com/midudev/autoskills)
