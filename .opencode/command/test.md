---
description: Ejecuta los tests de packages/skillmaxx con filtro opcional
---

Ejecuta los tests de `packages/skillmaxx`.
...
- Si no hay argumentos, ejecuta todos los tests:
  `pnpm --dir packages/skillmaxx test`
- Si hay argumentos, interprétalos como una pista flexible para elegir uno o varios archivos de `packages/skillmaxx/tests/*.test.ts`.
...
- Si hay una única coincidencia, ejecuta solo ese archivo:
  `pnpm --dir packages/skillmaxx exec node --test tests/<archivo>.test.ts`
...
```bash
rg --files packages/skillmaxx/tests -g "*.test.ts"
```
